...
def parse_arguments():
    ...
    parser.add_argument(
        "args",
        nargs="*",
        help="Additional arguments required by the specific command."
    )
...
def parse_email_entries(args_list):
    ...
    for entry_text in entries:
        ...
        print(f"Warning: Could not parse entry format: '{original_entry}' - Skipping this entry.")
...
def parse_name(full_name):
    ...
    if num_parts == 1:
        ...
        return {"first_name": name_parts[0], "middle_names": "", "last_name": name_parts[1]}
...
def check_emails(pattern):
    ...
    print(f"Checking for pattern '{pattern}' in database '{os.path.basename(db_path)}':")
    ...
    for entry in all_emails:
        ...
        if regex.search(entry.get("email", "")) or regex.search(entry.get("full_entry", "")):
...
def create_new_database(db_name):
    ...
    if os.path.exists(db_path):
        print(f"Error: Database '{db_filename}' already exists in '{DATABASE_FOLDER}'. Use a different name or delete the existing file first.")
        return False
...
def delete_database(db_name):
    ...
    if not os.path.exists(db_path):
        print(f"Error: Database '{db_filename}' not found in '{DATABASE_FOLDER}'. Cannot delete.")
        return False
    ...
    try:
        ...
        if config.get("active_database") == db_filename:
            ...
            if not os.path.exists(default_db_path):
                print(f"Default database '{default_db_filename}' not found. Creating it.")
...
def activate_database(db_name):
    ...
    if not os.path.exists(db_path):
        print(f"Error: Database '{db_filename}' not found in '{DATABASE_FOLDER}'. Cannot activate.")
...
def add_emails_to_database(emails_to_add):
    ...
    if db_data is None:
        ...
        if active_db_filename == default_db_filename:
            print(f"Default database '{default_db_filename}' not found or invalid. Attempting to create it.")
            if create_new_database(DEFAULT_DATABASE_NAME):
                ...
                print("Error: Failed to create the default database. Cannot add emails.")
                return False
        else:
    ...
    for new_entry in emails_to_add:
        ...
        if email_addr and email_addr.lower() not in existing_emails_lower:
            ...
            print(f"Warning: Skipping entry with missing email address: {new_entry.get('full_entry', 'Invalid Entry')}")
            skipped_count += 1

    # If, after filtering, there are no new emails to add, report and exit.
    if not newly_added_entries:
        print("No new emails to add."
              + (f" Skipped {skipped_count} duplicate/invalid entries." if skipped_count > 0 else ""))
    ...
    for email_to_add in newly_added_entries:
        ...
        if not added_to_existing_batch:
            ...
            print(f"Created new batch (ID: {new_batch_id}) for {email_to_add.get('email')}.")
    ...
    try:
        ...
        print(f"Successfully added {added_count} new email(s) to '{os.path.basename(db_path)}'.")
        if skipped_count > 0:
...
def remove_email_from_database(email_to_remove):
    ...
    if db_data is None:
        ...
        print(f"Error: Cannot remove email. Active database '{os.path.basename(db_path)}' could not be loaded.")
        return False
    ...
    for i, batch in enumerate(batches):
        ...
        updated_emails = [
            entry for entry in emails_in_batch
    ...
    if indices_of_empty_batches:
        ...
        updated_batches = [batch for idx, batch in enumerate(batches) if idx not in indices_of_empty_batches]
    ...
    try:
        ...
        print(f"An unexpected error occurred while saving the database after removal: {e}")
        return False
